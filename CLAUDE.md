...
**MCP auto-continue:** After a game ends, Claude automatically continues playing if the user starts a new compatible game from the menu. The orchestrator tracks `claudeWasOpponent` across menu transitions — when a Claude game ends and the user navigates to the menu, pending `wait_for_turn` calls are preserved (not flushed). If the user selects a new game that supports external moves, Claude opponent mode is auto-set and waiters are notified with the new game state. If the user selects a non-compatible game or minimizes the panel, waiters are flushed with "ended". The `gameOverReported` flag ensures gameover state is returned once to Claude (so it can report the result), then subsequent `wait_for_turn` calls wait for a new game or exit rather than returning gameover repeatedly.
